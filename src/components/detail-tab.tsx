...
export const DetailTab = () => {
  const [checkList, setCheckList] =
    useState<CheckListState[]>(mockCheckListData);
  const [attachments, setAttachments] =
    useState<AttachmentState[]>(mockFileData);
        ...
        <List spacing={2}>
          {attachments.map((attachment, index) => (
            ...
            >
              ...
              >
                <ListIcon
                  as={handleFormatThumbnail(attachment.fileType)}
                  fontSize="32px"
                />
                <Stack spacing={0}>
                  <Link href="#" isTruncated color="blue">
                    {attachment.fileName}
                  </Link>
